...
# # Function to remove item from the user's cart
...
#     conn = sqlite3.connect('icecream.db')
#     c = conn.cursor()
#     c.execute('''SELECT flavors.name, cart.quantity, cart.flavor_id 
#                  FROM cart 
...
#                 if st.button(f"Remove {item[0]}", key=f"remove_{item[0]}"):
#                     remove_from_cart(item[2])
